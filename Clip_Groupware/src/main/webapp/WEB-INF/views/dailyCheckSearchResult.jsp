...
<body>
 <div class="modal fade" id="dailyCheckModal" tabindex="-1"
   ...
   <form action="./searchDailyCheckList.do" method="get" class="form-inline" id="searchForm">
       ...
       <button type="submit" class="btn btn-primary mt-2" id="searchButton">검색</button>
     </form>
</div>
			<table class="table table-hover" id="searchResultTable">
				...
				<tbody>
					<tbody>
				    <c:forEach var="SRlist" items="${searchResult}">
				        <tr>
				            <th scope="row">${SRlist.user_id}</th>
				            <td>${SRlist.user_name}</td>
				            <td>${SRlist.positions_name}</td>
				            <td>${SRlist.ranks_name}</td>
				            <td>${SRlist.dept_name}</td>
				            <td>${SRlist.daily_intime}</td>
				            <td>${SRlist.daily_outtime}</td>
				            <td>${SRlist.daily_status}</td>
